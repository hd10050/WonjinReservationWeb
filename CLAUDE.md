...
- **2026-08-28 (74) 완료 — procedures 10개 카테고리에 병원 시스템 섹션(k-wonjin.co.kr 이관, playwright-cli)**. 신규 `data/procedureMedical.ts`(블록 유니온 `intro`/`steps`/`quote`/`features`/`gallery`, 카테고리 slug 키, features 아이템에 `no?`·`imageFit?:'contain'`, quote에 `cite?`) + `components/ProcedureMedicalSystem.vue`("고민이 있으신가요?" 위, `bg-muted/30`, **블록별 스타거 페이드업** — `useScrollReveal` + `revealed` 상태값에 딜레이 직접 바인딩). 콘텐츠 있는 9개(nose·ent·dermatology·stemcell·breast·contour·bodyline·men·reconstruction), lifting은 헤딩만. 이미지 `public/img/{category}/`에 curl(CSP `img-src 'self'`). zh-CN/zh-TW/en 직접 번역. 헤딩: `ProcedureCategory.concernSubject?`(lifting→피부 탄력, contour→얼굴형, ent→코, stemcell→피부 탄력) + `concernNoPrefix?`(dermatology·men·reconstruction) + i18n `procedures.concernHeadingNoPrefix` 4로케일. reconstruction award png는 4000px 투명캔버스라 갤러리 제외. `npm run build` 0에러, playwright-cli 스크린샷 전 페이지 확인. 상세 `docs/session-log.md`.
