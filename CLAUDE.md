...
- **2026-08-26 (26) — 예약 폼 날짜·시간 입력 UI 다국어화 + 연락희망시각 라벨에 UTC+9 명시**: 사용자가 "다국어로 폼 조작 시 생년월일·연락희망시각 UI가 한글로 나옴"을 재현 보고 → 실측 결과 `<html lang>`은 로케일별로 정상인데 개별 `<input type="date/time">`에는 `lang` 속성이 없었음(디폴트 undefined). `index.vue`에 `inputLang` computed(`nuxt.config.ts`의 `locales[].language` BCP-47 태그) 추가해 두 입력에 `:lang` 바인딩 — 4로케일 전부 `<html lang>`과 값 일치 브라우저 실측 완료. 🔴 **팝업 달력 자체(요일·월 이름)는 이 속성과 무관하게 브라우저/OS 언어를 따르는 네이티브 위젯이라 코드로 제어 불가**(D11에 따라 JS 라이브러리 미도입 원칙 유지, 정확한 브라우저별 동작은 `[미확인]`) — design.md 9-2절에 한계 명시. `landing.form.contactTime` 라벨의 "(한국 시간)"을 "(한국시간 UTC +9)"(ko)/각 언어 등가 표현(UTC+9 병기)으로 4로케일 전부 갱신, 브라우저 실측으로 각 로케일 라벨 텍스트 확인. i18n 276키 4파일 동일 유지. `npm run build` 클린. 커밋+push 완료.
