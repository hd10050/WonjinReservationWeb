...
| 8 | ✅ 유입 경로 분석(`phase8-referral-analytics` 워크트리, 2026-08-26 구현+실측 완료, **main 미병합**) | 비어드민 접근 차단 실측 — **완료**: curl 4종(Admin 200 / HospitalManager 403 / Consultant 403 / 익명 401) + 브라우저 3역할 전건(Admin만 진입, 나머지 `/admin` 리다이렉트) 확인 |
...
- **2026-08-26 (21) — Phase 8(유입 경로 분석) 구현+실측 완료, `phase8-referral-analytics` 워크트리(병합 대기)**: 다른 세션과 섞이지 않도록 사용자 지시로 신규 워크트리를 파서 진행(병합·워크트리 정리는 명시적 지시 전까지 보류). design.md 전문 재독 결과 Phase 8 스펙(D4·D5·11-5·15-2·12-7절)이 이미 확정돼 있어 별도 설계 없이 바로 구현. 백엔드 `AdminStatsController`에 `GET /referrals`(어드민 전용 — 클래스 레벨 `Admin,HospitalManager`를 액션 레벨로 좁힘, 6-3절 원칙) 추가 + `ReferralStatDto` 신규 — `landing_daily_stats`를 조합별로 그룹핑해 기준 집합으로 삼고 같은 조합의 `reservations` 건수를 매핑(11-6절 2단계 집계 패턴 재사용). 프론트 `/admin/referrals` 신규(표만 — 12-7절에 이 메뉴는 차트 요구 없어 D21 대상 아님) + `types/reservation.ts`·4로케일 14키 신규(219키 4파일 동일 확인). 인덱스는 Phase 0에 이미 존재(`ix_landing_daily_stats_stat_date`·`ix_reservations_created_at`, 8-5절에 이 용도로 명시)해 신규 마이그레이션 없음. **격리 docker 스택**(postgres 5435/api 5201/frontend 3700 — 다른 세션이 이미 api 5200을 점유 중이라 `docker-compose.override.yml`의 `ports: !override`로 충돌 회피, design.md 4-2절 문서화된 기법 그대로)에 테스트 계정 3개(admin/manager/consultant, `TestPassword123!`)+시드 데이터로 curl 4종(Admin 200·HospitalManager 403·Consultant 403·익명 401, 응답값 수동 계산과 정확히 일치) + 브라우저 3역할 전건 실측(Admin만 진입 성공해 표 데이터 정상 렌더, HospitalManager·Consultant 둘 다 `/admin`으로 리다이렉트) 후 스택·스크래치 파일 정리. `dotnet build`(0경고 0오류)·`npm run build` 둘 다 성공.
