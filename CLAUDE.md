...
- 현재 상태: **Phase 1~3 구현 완료·main 병합 완료**(2026-08-26). **Phase 4(실장·시술 관리 CRUD) 구현+실측 완료**(2026-08-26) — 단 **워크트리 브랜치 `worktree-phase4-consultants-procedures`에 커밋 완료 상태이며 main 미병합, 병합은 사용자 지시 대기**. 동시에 Phase 5(예약 달력)·Phase 6(실장 KPI·예약 통계)이 각각 별도 워크트리에서 병렬 진행 중 — 3개 브랜치 병합 시 `frontend/i18n/locales/*.json` 4파일과 `layouts/admin.vue`(현재 사이드바 없음, 로고+로그아웃 헤더만)가 충돌 가능 지점. **사이드바 네비게이션(12-3절)은 각 Phase가 개별 구현하지 않고 3개 브랜치 병합 시점에 한 번에 정리하기로 사용자 결정**(2026-08-26) — 그 전까지 `/admin/consultants`·`/admin/procedures`·달력 등 신규 화면은 URL 직접 접근으로만 확인 가능. Phase 7부터는 사용자 지시 대기
