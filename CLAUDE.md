...
- **2026-08-28 (74) 진행중 — procedures 카테고리 페이지에 병원 시스템·특장점 섹션 추가(k-wonjin.co.kr 이관, playwright-cli 사용)**. 신규 `data/procedureMedical.ts`(블록 유니온 intro/steps/quote/features, 카테고리 slug 키) + `components/ProcedureMedicalSystem.vue`("고민이 있으신가요?" 위 렌더, `bg-muted/30`+스크롤 리빌). `procedures/[category]/index.vue`에 `<ProcedureMedicalSystem v-if="medical">` 배선. **nose 먼저 완료**(innofit_2·3.png `public/img/nose/`, ko 원문+zh-CN/zh-TW/en 직접번역), 레이아웃 확인 후 나머지 9개(ent·lifting·dermatology·stemcell·breast·contour·bodyline·men·reconstruction) 일괄 예정 — 헤딩 문구 변경(`concernSubject`/no-prefix)도 그때. `npm run build` 0에러.
