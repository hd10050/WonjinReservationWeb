...
- **2026-08-28 (73) — 홈 하단 문의 CTA 리디자인 + 푸터 팔레트 색상 적용(사용자 지시)**. ①`pages/index.vue` 최하단 CTA: `bg-primary` 밴드+버튼 1개 → 헤딩+보조문구+버튼 3단 구성, 배경 `bg-gradient-to-b from-primary to-foreground`(올리브→짙은 산림녹), 버튼 `variant="secondary"` 유지, 형제 섹션과 동일 `useScrollReveal()` 추가. 신규 i18n 키 `landing.home.ctaHeading`·`ctaBody` 4로케일. ②`layouts/landing.vue` 푸터: `border-t bg-card` → `bg-foreground text-primary-foreground`(#283618, 팔레트 최암색), 링크 hover `text-primary-foreground`. CTA 그라디언트 끝색==푸터 배경색이라 자연 연결. `npm run build` 0에러, dev 서버 DOM 실측 후 main 커밋·push.
