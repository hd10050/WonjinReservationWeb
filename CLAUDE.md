...
- **2026-08-28 (57) — 유입경로 페이징·감사로그 검색UX·계정 본인이름수정·상담폼 rate limit 피드백·연락희망시각 상관없음 체크박스 5건, main 직접 작업, 상세는 `docs/session-log.md` (57)**: `GET /api/admin/stats/referrals`에 page/pageSize 추가(landing_daily_stats 그룹 쿼리에 Skip/Take, `PagedResult`). 감사로그 검색 엔터키 제출 + 적용/초기화를 필터 아래 고정 줄로 분리(필터 개수 따라 위아래로 움직이던 버그 수정). `AdminUsersController.Update`에 Name 필드 추가(본인은 이름만, 역할·정지는 여전히 차단 — 위 주의사항 참고). 예약 폼 rate limit **1분당5건→5분당5건**(사용자 지시) + 429 응답 `{code:RATE_LIMITED}` 구조화(`OnRejected`, 기존 화면들의 `e.data.code` 패턴 그대로 재사용). `preferred_contact_time`을 nullable로 마이그레이션 후 "상관없음" 체크박스 신설(D26). `dotnet build`·`npm run build` 0경고/0에러, i18n 4파일 407키(+4) 동일 확인. **격리 docker 검증**(`wonjin-verify`, 별도 포트·볼륨·프로젝트명, 기존 공유 스택 무변동): 유입경로 25건→2페이지 정상, 감사로그 엔터검색+버튼 480px 폭에서도 고정, 계정 본인/타인 이름수정 저장 확인+본인 role변경 API 직접호출로 CANNOT_MODIFY_SELF 재확인(세션 유지), 예약폼 상관없음 체크→필드 disabled 확인+DB null 저장 확인+어드민상세 "상관없음" 렌더 확인+미체크 시 기존 검증 그대로 유지 확인, rate limit 5건 소진 후 6번째 429+RATE_LIMITED 바디 확인. 검증 후 볼륨 포함 완전 삭제.
