...
- [ ] **로컬 DB 테스트 더미 예약 정리 여부 확인** — Phase 2 실측 검증(동시성 20건·rate limit·honeypot·4언어 브라우저 E2E 등) 중 생성된 더미 `reservations` **30건**이 로컬 dev DB에 남아있음. 실서비스 데이터 아님, Phase 3에서 대시보드 목록 확인 전에 정리 권장
- [ ] **관리자 헤더 로고(`layouts/admin.vue`) 시각 미검증** — 코드는 배포됨(랜딩 헤더와 동일한 `<img src="/logo.svg">` 패턴, 빌드 성공 확인)이나 브라우저 자동화 도구의 로그인 클릭이 반응하지 않아 `/admin` 로그인 후 화면 실측은 못 함. 다음 접속 시 육안 확인 권장
...
- [ ] **M12 OG 공유 이미지** — 현재 로고(`favicon.png` 32×32, `logo.svg`)는 소셜 미리보기용으로 부적합(해상도 낮음·SVG 다수 플랫폼 미지원, `[미확인]`). 1200×630 권장 PNG/JPG 별도 제작 필요, Phase 9
