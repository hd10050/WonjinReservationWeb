...
- **"존재확인만 하는 별도 SELECT" 뒤에 INSERT하는 패턴은 동시 소프트삭제와 레이스한다(D15)** — `AddNote`가 배정여부를 트랜잭션 밖 SELECT로만 확인하던 것을, 조건부 `ExecuteUpdateAsync`(행 lock 효과)로 바꿔 SoftDelete와 직렬화되게 수정(보안감사 2026-08-26). 새 쓰기 액션을 추가할 때 이 컨트롤러의 다른 5개 액션(원자적 WHERE 패턴)과 다른 방식을 쓰고 있지 않은지 확인할 것. RT 1회용 로테이션 환경에서 SSR의 여러 데이터 페칭 지점이 각자 독립 refresh하면 안 되는 함정도 같은 감사에서 발견해 `useState` 요청스코프 캐시(`ssrRefreshCookie`, `useAuth.ts`)로 해결 완료(세션요약 (27) 참고)
...
| 9 | SEO·보안감사·배포 | 라이브 curl 검증 — **인프라 배포는 2026-08-26 완료**(Cloudflare Workers+Render). **보안감사 1라운드**(High2·Medium5 수정, `AssignConsultant` RowVersion까지 2026-08-27 완전해결) + **SEO 파트** **로컬 검증까지 완료**(세션요약 (27)(28)(30)). **Low 12건은 원래 목록이 어디에도 저장 안 돼있던 것을 발견**(2026-08-27) — 재감사 진행 중(세션요약 참고) + 실배포 도메인 대상 라이브 curl 검증 미착수 |
...
## 세션 요약 (오래된 항목은 `docs/session-log.md` 참고, (28)까지 이동 완료)
