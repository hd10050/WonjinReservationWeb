...
- **2026-08-27 (43) — "로그아웃 시 다른 탭 즉시 로그아웃 안 됨" 수정 — `auth-pattern-reference.md` 6-7절 `BroadcastChannel` 패턴 이식, main 커밋 `e0464b2`+(44) 순서정정 `a3db59f` push 완료**: `useState('auth:user')`가 탭마다 독립이라 한 탭 로그아웃이 다른 탭에 전파 안 되던 것이 원인. [useAuth.ts](frontend/app/composables/useAuth.ts)의 `logout()`에 `BroadcastChannel('wj_auth')` 방송 추가 + 신규 [plugins/02.auth-sync.client.ts](frontend/app/plugins/02.auth-sync.client.ts)가 수신해 `user.value=null`만 설정(리다이렉트는 기존 `admin.vue`의 `watch(user)`가 그대로 담당, 중복 네비게이션 없음). 로그인 동기화(`type:'login'`)는 요청 범위 밖이라 미구현.
...
- **2026-08-27 (45) — "로그인이 느림" 재조사(이번엔 로그인 *페이지 자체*가 원인) — `work/session-0827` 워크트리 작업, 병합 대기(사용자 지시 전까지 미병합)**: (42)는 로그인 *이후* `/admin` 대시보드의 지연을 고쳤으나 이번 재현은 로그인 폼이 뜨기 *전* 지연이었음. 원인 — [01.auth.ts](frontend/app/plugins/01.auth.ts)의 `path.startsWith('/admin')` 가드가 `/admin/login`도 관리자 경로로 취급해, 미인증 방문자에게 항상 401로 실패하는 `fetchMe()`(SSR은 `ssrRefreshCookie()` refresh까지 재시도)를 로그인 폼 렌더 전에 블로킹 실행 중이었음(`middleware/admin.ts`는 이미 로그인 페이지 제외, 이 플러그인만 누락) — `path === '/admin/login'` 조건 추가, `docs/design.md` 예제 코드도 동기화. 부수 수정: [RefreshTokenService.cs](api/Services/RefreshTokenService.cs) `CreateAsync()`가 로그인·12분마다 토큰갱신 요청마다 실행하던 정리용 DELETE 제거(`RefreshTokenCleanupService`가 12시간마다 이미 동일 정리 수행 — 중복 DB 왕복이었음). **검증**: 격리 docker(`wonjin-verify-login`, 기존 공유 스택 무영향 확인 후 완전 삭제)에서 `Microsoft.AspNetCore` 로그를 Information으로 올려 실측 — 수정 전 `/admin/login` 3회 요청 → 백엔드 `/api/auth/me` 401 3회 기록, 수정 후 동일 3회 요청 → 백엔드 호출 0건(로그 라인 증가 없음). 로그인 POST(BCrypt workFactor 12) 자체는 이번에도 ~240ms로 정상 재확인(기존 결론과 일치, 변경 없음). `dotnet build`·`npm run build` 둘 다 0에러, 로그인→GET me→POST refresh(RT 로테이션 확인)까지 e2e curl로 회귀 없음 확인.
- **2026-08-27 (46) — 4건 UI 버그·개선 (TimePicker 미작동·DatePicker 폭/기본값·상세페이지 로딩오버레이·실장필터 리셋) — `work/session-0827` 워크트리, 병합 대기**: ①**TimePicker 렌더 누락**(근본원인) — `<TimeFieldInput part="hour" />`가 자식 없이 쓰여 있어 세그먼트 값이 내부적으로(aria-valuenow)는 갱신돼도 화면 텍스트는 영원히 빈 칸이었음(Context7 reka-ui 공식 문서로 `segments` 스코프드 슬롯 기반 렌더 패턴 확인 후 [TimePicker.vue](frontend/app/components/ui/time-picker/TimePicker.vue) 수정, `dayPeriod` 세그먼트가 h23 지정에도 섞여 나오는 것도 함께 필터링) + 폭 `w-fit`→`w-32`. 🔴 실측 중 자정(00:xx) 표시가 내부값과 달리 "12"로 보이는 별도 결함 발견 — 위 TODO 3번 항목 참고, 실브라우저 재확인 전까지 미수정. ②**DatePicker 폭**을 컴포넌트 자체에 `w-[200px]` 기본값으로 넣고 7개 페이지 12곳의 개별 `w-40` 제거(단일 소스, 요청한 200px 정확히 반영) + **[audit-logs.vue](frontend/app/pages/admin/audit-logs.vue)만 날짜 필터 기본값이 없던 것**을 발견해 나머지 4페이지와 동일한 당월1일~현재로 통일. ③**예약 상세 페이지**([id].vue) 쓰기 액션 7개(배정·상담기록추가/수정·저장·상태전이2종·복구) 전부가 공유하는 `busy` 플래그 + RouteOverlay와 동일한 상시마운트·class바인딩 방식(Transition 금지, 13-2절)의 전체화면 스피너 오버레이 추가. ④**대시보드 실장필터**([index.vue](frontend/app/pages/admin/index.vue)) — "비활성 포함" 체크 해제로 목록이 바뀌어 선택값이 사라지는 케이스를 `watch(consultants, ...)`로 감지해 "전체"로 리셋(비활성 토글뿐 아니라 목록이 바뀌는 모든 경우에 적용). **검증**: `npm run build` 0에러(수정 5회 반복, 매번 재확인) + 격리 docker(`wonjin-verify-ui`, 종료 후 삭제·기존 스택 무영향 확인)에서 실제 로그인 후 4건 전부 브라우저 실측 — 비활성실장 선택→해제 시 select값 `""`로 정확히 리셋, 날짜필터 12곳 전부 200px+올바른 기본값, 방문확정 액션 클릭 시 오버레이 `pointer-events-auto`→완료 후 `pointer-events-none` 전환 확인.
- **2026-08-27 (47) — 미작업분 점검 중 (46)의 TimePicker 자정 표시 결함 근본원인 확정·해결 — `work/session-0827` 워크트리, 병합 대기**: `node_modules/reka-ui/src/shared/date/utils.ts`의 `normalizeHourCycle()`을 직접 읽어 확정 — 이 함수는 `hourCycle === 24`/`=== 12`(숫자 엄격비교)만 인식하고 **문자열은 전부 `undefined`로 떨어져 로케일 기본 hourCycle로 조용히 폴백**된다. 즉 `hour-cycle="h23"`(문자열)은 사실상 아무 효과가 없었던 것 — 기존 CLAUDE.md 원칙("문자열이 맞다")이 **오판이었음을 확인해 정정**(위 106절 참고, 근거였던 `aria-valuemax=23`은 이 문제와 무관한 별도 코드경로였음). `:hour-cycle="24"`(숫자)로 수정. 부수 발견: `hourSegmentAttrs`의 `segmentValues.hour ? ... : placeholder`가 falsy-zero 버그라 `aria-valuenow`는 자정일 때 신뢰 불가 — 검증은 반드시 화면 텍스트(`textContent`)로 할 것(합성 이벤트 실측이 매번 다르게 나왔던 진짜 원인). **검증**: `npm run build` 0에러 + 격리 docker(`wonjin-verify-tp`, 종료 후 삭제)에서 화살표키로 자정→화면 텍스트 정확히 "0", 13시→"13" 확인 + Vue 인스턴스 내부 상태 직접 조회로 부모 `v-model`(`contactTime`) 값이 정확히 `"00:00"`임을 확인(aria 속성이 아닌 실제 데이터로 검증). TODO의 "실브라우저 확인 필요 3건" → 2건으로 정리.
- **2026-08-27 (48) — 개인정보 처리방침 모달이 좌측 상단에 붙는 결함 수정 — `work/session-0827` 워크트리, 병합 대기**: 원인은 Tailwind Preflight(`@import "tailwindcss"`)가 전 요소 `margin`을 0으로 리셋하는데, 네이티브 `<dialog>`의 중앙 정렬은 브라우저 기본 스타일시트의 `margin:auto`가 담당해 author 스타일시트(Preflight)가 항상 이겨 무력화된 것 — [index.vue](frontend/app/pages/index.vue) dialog에 `m-auto` 추가로 복원(Tailwind utilities 레이어가 base 레이어보다 뒤에 와 항상 이김). 코드베이스 전체에 네이티브 `<dialog>` 사용처가 이 한 곳뿐임을 grep으로 확인(다른 곳에 동일 결함 없음). **검증**: `npm run build` 0에러 + 격리 docker(`wonjin-verify-modal`, 종료 후 삭제)에서 `m-auto` 있음/없음 두 빌드를 실제로 교차 배포해 대조 — 수정 전 `getBoundingClientRect()` `{x:0,y:0}`(정확히 좌측 상단, 신고 내용과 일치) → 수정 후 뷰포트(1280×900) 중앙 대비 오차 `{x:8px, y:0px}`. 열기→X닫기→재열기 흐름도 회귀 없음 확인(과거 (36) 세션의 "닫기 후 재열기 안 됨" 사고류 재발 없음).
- **2026-08-27 (49) — [예약 달력] 날짜 클릭 전 42일치 전체 로드를 2단계 API로 분리 + 우측 목록 스크롤 캡 — `work/session-0827` 워크트리, 병합 대기**: 사용자가 "날짜 클릭 전엔 그 날짜 데이터 로드 안 되는 게 맞지 않냐"고 지적, 설계 방향 논의 후 승인받아 진행. 기존 `GET /calendar?year=&month=`가 42일 전체 예약 **상세**를 한 번에 반환해 그리드가 쓰는 배지 숫자 하나 때문에 날짜 클릭 전부터 모든 날의 상세가 로드돼 있었음. ①`GetCalendar`를 `GroupBy(visit_date).Count()`로 바꿔 **날짜별 건수만** 반환(그리드 배지 전용, `GroupBy(...).Select(g=>new Dto(...))` 직결 금지 함정은 익명 타입 선집계로 우회) ②신규 `GET /calendar/day?date=`를 추가해 상세 목록은 날짜를 클릭한 시점에만 그 하루치만 반환 — 둘 다 동일 부분 인덱스 `ix_reservations_visit_date` 사용. 프론트 [calendar.vue](frontend/app/pages/admin/calendar.vue)는 `selectedDate`를 `useApi`의 반응형 query로 감시하는 두 번째 호출을 추가해, 최초 진입 시엔 SSR로 함께 프리로드(화면 깜빡임 금지 준수)되고 이후 날짜 클릭·월 이동 때만 재조회되도록 함(클릭 핸들러가 직접 fetch 호출 안 함, admin/index.vue의 반응형 조회 패턴과 동일). 우측 "선택한 날짜의 예약 목록" 카드에 `max-h-[70vh] overflow-y-auto` 추가(사용자 요청). **검증**: `dotnet build`·`npm run build` 둘 다 0에러 + 격리 docker(`wonjin-verify-cal`, 종료 후 삭제·기존 스택 무영향 확인)에서 8월 15일 40건·20일 3건 시드 후 백엔드 요청 로그로 실측 — 최초 페이지 로드 시 `calendar`(건수)+`calendar/day?date=오늘`(0건) 딱 2건만 호출되고 15·20일 상세 요청은 전혀 없음 확인, 15일 클릭 시 그제서야 `calendar/day?date=2026-08-15` 요청 1건 발생(20ms 응답) 확인, 그리드 배지 40/3 정확 확인, 40건 렌더 시 `scrollHeight(2552px) > clientHeight(630px)`로 실제 스크롤 발생 확인. `docs/design.md` 12-6절·성능 쿼리표 동기화 완료.
