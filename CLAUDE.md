...
## 세션 요약 (오래된 항목은 `docs/session-log.md` 참고, (74)까지 이동 완료)
...
- **2026-08-30 (75) — auth/security/SEO 3개 공유가이드 대비 전수 감사 + 결함 5건 수정(전부 Low)**. 백엔드(컨트롤러 12·필터 2·서비스·Program.cs·모델·DTO·마이그레이션 스냅샷) + 프론트(인증 컴포저블/플러그인/미들웨어·SEO·프록시/CSP) 전수 정독. **대부분 결함 없음** — 토큰설계·RT해시회전·정지/강등 실시간무효화(`AccountStateFilter`)·CSRF(Origin+내부시크릿)·IP신뢰체인(커스텀 `X-Wj-Client-Ip`+XFF제거, 라이브검증)·RBAC 액션레벨 재좁힘·SQLi(파라미터+`LikeEscape`)·입력 3곳일치·XSS(JSON-LD 이스케이프)·시크릿관리 전부 정상, 가이드가 경고하는 함정을 오히려 선제 차단. **수정 5건**: ①홈 `<meta description>` 누락 → `landing.home.seoDescription` 4로케일 + `useSeo` 반영 ②시술 카테고리/상세 페이지 페이지단위 구조화데이터 없음 → `useSeo`에 `schemaOrg` 옵션 추가(`MaybeRefOrGetter` + 게터 래핑 + `<`→`<`) + `MedicalWebPage`/`MedicalProcedure` JSON-LD 주입("그 외" 항목은 noIndex라 스키마 제외) ③언어 리다이렉트 `/zh-tw`가 canonical/hreflang `/zh-TW`와 대소문자 불일치 → `nuxt.config` 인라인스크립트·`server/routes/go/[code]` 프리픽스를 i18n `code`(zh-TW)로 통일 ④불필요한 `frontend/public/_robots.txt` 삭제(@nuxtjs/robots 설정이 이미 담당, 라이브 mockProd robots.txt에 `Disallow: /admin/` 유지 확인) ⑤SSE `/api/admin/events`가 `AccountStateFilter`(MVC 액션필터) 밖 → minimal API 핸들러에 연결시점 `IsSuspended`+토큰Role 재검사 추가(EventSource 재연결마다 재검증, 페이로드는 이미 예약ID뿐). api `dotnet build` 0에러 · frontend `nuxt build` 0에러 · 라이브 SSR curl로 메타·JSON-LD(상세 2블록)·robots·noindex 전건 확인. 커밋 `0e65bcc`.
