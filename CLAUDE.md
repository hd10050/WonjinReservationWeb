...
- 🔴 **`CF-Connecting-IP`는 "프론트(Cloudflare Workers) 뒤"에서만 위조 불가능** — 백엔드(Render)는 Cloudflare 엣지 뒤가 아니라서, 이 헤더를 무조건 신뢰하면 동일 출처 프록시를 건너뛰고 백엔드를 직접 호출하며 헤더를 조작해 Rate Limit을 무제한 우회할 수 있다(보안감사 2026-08-26 발견). 프론트·백엔드만 아는 내부시크릿(`X-Internal-Secret`)이 유효할 때만 신뢰하고, 아니면 실제 TCP 연결 IP로 폴백(`Program.cs GetClientIp`, `AuditLogFilter`). 🔴 **프록시가 백엔드로 relay하는 헤더 이름 == 백엔드가 읽는 헤더 이름을 반드시 코드로 대조**(2026-08-28 (61) 수정) — `server/api/[...].ts`가 `x-forwarded-for`로 보내는데 `GetClientIp()`는 `CF-Connecting-IP`만 읽어 전 방문자가 한 버킷으로 뭉개져 있었다. 프록시가 `cf-connecting-ip`로 relay하도록 수정, IP별 독립 버킷 curl 2단계 실측(같은 IP 6회째 429 / 새 IP 즉시 통과) — **로컬 검증 완료, 실배포(Cloudflare→Render) 라이브 curl 2단계 재확인 필요**(memory: [[rate-limit-trusted-ip-only]])
...
## TODO
### 다음 세션 최우선
- [ ] 🔴🔴 **IP 레이트리밋, 실배포 라이브 curl 재확인 결과 여전히 미작동(2026-08-28 (62))** — `wonjinreservationweb.hd1005019.workers.dev/api/reservations`에 honeypot 채운 안전 payload로 8회 연속 POST(같은 실IP) → **8회 전부 200, 429 없음**(정책은 5분당 5회 제한). `CF-Ray` 콜로가 요청마다 PDX/LAX로 바뀌는 것으로 보아 백엔드가 `CF-Connecting-IP`를 못 믿고 `RemoteIpAddress`(Cloudflare 엣지의 가변 아웃바운드 IP)로 폴백 중일 가능성 높음 — 원인 후보: ①Render `INTERNAL_SECRET`/프론트 `NUXT_INTERNAL_SECRET` 값 불일치 ②Render가 아직 커밋 `5b4c651`(헤더이름 수정) 이전 빌드로 떠있음(배포 지연/실패). 다음 세션에서 Render 배포 로그의 실제 반영 커밋 SHA부터 확인할 것. memory: [[rate-limit-trusted-ip-only]]
- [ ] **인플루언서 링크 방문 집계, 부분 검증(2026-08-28 (62))** — `/go/{존재안하는코드}` 라이브 302 리다이렉트 자체는 정상(`Location: /?ref=코드`). 단 **실제 존재하는 코드로 방문 시 `landing_daily_stats` 카운트가 실제로 오르는지는 미검증**(관리자 로그인·유효 인플루언서 코드 필요, 이번 세션엔 둘 다 없어 보류) — 다음 세션에서 실제 코드로 `/go/{code}` 방문 전후 `/admin/유입 경로 분석` 수치 대조 필요
...
## 세션 요약 (오래된 항목은 `docs/session-log.md` 참고, (60)까지 이동 완료)
- **2026-08-28 (61)** — 3건 수정, main 직접 작업(커밋 `0241597`·`5b4c651`·`ff12358`·`1fc8d77`). ①**계정 발급 폼 콘솔 경고** — `admin/users.vue`의 password `<input>`이 `<form>` 밖이라 Chrome "Password field is not contained in a form" 경고 → 생성 폼을 `<form novalidate @submit.prevent>`로 감싸고 저장 버튼 `type="submit"`, email `autocomplete="username"`·password `autocomplete="new-password"` 추가. 브라우저 실측: `#f-password.closest('form')` 존재·폼 제출로 계정 생성 왕복 확인. ②**IP 레이트리밋 헤더불일치**((58) 발견분) — 프록시 `server/api/[...].ts`가 `x-forwarded-for`로 IP를 보내는데 백엔드 `GetClientIp()`는 `CF-Connecting-IP`만 읽어 전 방문자가 한 버킷. 프록시가 `cf-connecting-ip`로 relay(`getRequestHeader('cf-connecting-ip')` 우선, 없으면 `getRequestIP`)하도록 수정(백엔드 무수정). curl 2단계 실측: 같은 IP 6회째 429 / 새 IP 2종 즉시 통과. **실배포 재확인 TODO**. ③**인플루언서 링크 방문 미집계 재조사** — (60)은 `/go/{code}` 조회 timeout만 고쳤으나 정작 `landing_daily_stats`에 쓰는 건 `layouts/landing.vue`의 `landing-visit` 호출(`timeout:2000` + `await` 없는 fire-and-forget). **근본 원인**: Cloudflare Workers는 SSR 응답 반환 시 `waitUntil` 미등록 백그라운드 fetch를 종료할 수 있음(Workers 규정 동작). `timeout:10000` + `nuxtApp.ssrContext?.event?.waitUntil(visitTask)`로 수정(`useRequestEvent()`를 if 블록 안에서 늦게 부르면 NUXT_E1001 → setup 최상단 `useNuxtApp()`로). 로컬 end-to-end 실측: `/go/{code}` → 302(UTM 포함) → 랜딩 SSR → `landing_daily_stats` 행 생성·재방문 시 count 증가. 로컬(Node)은 응답 후 프로세스가 살아 유실 재현 안 됨 → **실배포 재확인 필요**. design.md 7-5·15-1절 갱신. ④사용자 지시로 "Render 콜드스타트 언급 금지" 규칙 CLAUDE.md 명문화 + (60)(61) 문구·주석에서 콜드스타트 표현 제거(memory: [[feedback_no_render_coldstart_excuse]]). DB 테스트데이터 잔여 0건.
- **2026-08-28 (62)** — (61) 실배포 재확인 라이브 curl(코드 변경 없음, 검증만). ①**IP 레이트리밋 — 여전히 미작동**: `honeypot` 채운 안전 payload로 실도메인에 8회 연속 POST(같은 실IP) → 8회 전부 200(정책은 5분당 5회 제한, 8회째 429 기대). `CF-Ray` 콜로가 요청마다 PDX/LAX로 바뀌는 것으로 미루어 백엔드가 `CF-Connecting-IP`를 못 믿고 `RemoteIpAddress`로 폴백 중일 가능성. 코드는 로컬 검증대로 맞아 보이므로 다음 용의선상은 Render 실제 반영 커밋(빌드 지연/실패) 또는 `INTERNAL_SECRET` 값 프론트·백엔드 불일치 — 코드 재작성 아니라 배포 상태 확인부터. ②**인플루언서 링크 방문 집계 — 부분 검증**: `/go/{존재안하는코드}` 라이브 302(`ref=코드` 보존)는 정상. 유효 코드로 실제 카운트가 오르는지는 관리자 로그인·실제 발급된 코드가 없어 미검증, 다음 세션 과제로 이월. ③상담기록 "수정 이력" 모달 전환은 사용자 확인으로 **보류 결정**(현재 인라인 패널 유지) — TODO에서 제거. ④랜딩 실제 백엔드 왕복(폼 제출→DB) 검증 항목은 우선순위 낮음 판단으로 TODO에서 제거(필요시 재등록).
