...
## TODO
### 다음 세션 최우선
- [ ] 🔴 **(61) 수정 2건 실배포 재확인** (둘 다 로컬은 통과, 실배포 Cloudflare Workers→Render 경로만 미검증): ①**IP 레이트리밋 헤더불일치** — 서로 다른 `CF-Connecting-IP`가 실제로 독립 버킷을 받는지 workers.dev URL에 라이브 curl 2단계(같은 IP 한도초과→차단 / 새 IP 즉시 통과), memory: [[rate-limit-trusted-ip-only]] ②**인플루언서 링크 방문 집계** — `/go/{code}` 접속 후 `/admin/유입 경로 분석`에 방문수가 잡히는지(landing.vue `waitUntil` 수정이 실배포 Workers에서 실제로 방문 유실을 막는지)
