...
- 현재 상태: **Phase 1~9 전부 완료**(2026-08-27, 인프라 실배포 프론트 Cloudflare Workers `wonjinreservationweb.hd1005019.workers.dev` / 백엔드+DB Render `wonjinreservationweb.onrender.com`). 이후 여러 세션에 걸쳐 랜딩 전면 재설계·카테고리 마스터 개편(D25)·연락희망날짜/DatePicker/계정폼 결함 수정 등 다수 완료(세션요약 (35)~(63), 상세는 `docs/session-log.md`). **2026-08-28 (64)~(66)**: IP 레이트리밋·웹푸시 알림 전수감사(신규 결함 0건) + 어드민 웹푸시 토글 Switch+Tooltip 재구현 + 실브라우저 확인 2건(웹푸시 OS 알림·Popover 애니메이션) 종결 + **CSP 구현+실배포 라이브 curl 검증까지 완료**. **남은 건 랜딩 시술 이미지 6개 누락·중국어 고민문구 출처확인(아래 TODO) 뿐**
...
- **2026-08-28 (66)** — **실브라우저 확인 2건 종결(사용자 직접 확인) + CSP 구현+실배포 검증까지 완료**. CSP는 script-src nonce 방식: `landing.vue` JSON-LD가 실제로는 완전 고정 문자열임을 코드로 재확인해 옛 "예약마다 달라 해시 불가" 전제가 틀렸음을 발견했지만, 진짜 걸림돌은 Nuxt SSR 하이드레이션 페이로드(매 요청 실데이터 포함, 정적 해시 불가)였음. `server/middleware/csp-nonce.ts`(요청별 nonce 생성+CSP 헤더) + `server/plugins/csp-nonce-html.ts`(Nuxt 4 공식 `render:html` 훅으로 전 `<script>`에 nonce 부여, Context7로 확인) 신설. dev·`node-server` 프로덕션 빌드 로컬 curl 검증 후, 사용자가 production 병합 → 실배포 도메인(`wonjinreservationweb.hd1005019.workers.dev`)에서 CSP 헤더·전 스크립트(하이드레이션 페이로드·JSON-LD·importmap) nonce 부여·매 요청 nonce 재생성·`/admin/login` 동일 헤더·브라우저 콘솔 위반 0건까지 라이브 curl+실측 완료. design.md 19장 Phase 9 완료 기준("라이브 curl 검증") 충족, **Phase 1~9 전 항목 완료**.
