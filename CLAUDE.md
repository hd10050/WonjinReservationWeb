...
- [ ] **`nuxt.config.ts`에 `emitRouteChunkError: 'reload'` 누락**(세션 종료 전 미작업분 점검 중 신규 발견, 2026-08-27, 실측 확인 — 아직 미수정) — auth-pattern-reference.md 8장 체크리스트 항목인데 보안감사 5번 에이전트가 "파일 목록 밖"이라 `[미확인]`으로 남긴 채 아무도 재확인 안 하고 넘어갔던 것. 재배포 후 열려있던 탭이 구 JS 청크 404를 맞으면 auth 플러그인이 실행 안 돼 "로그인이 풀린 것처럼" 보이는 버그(버그#4) 가능 — Cloudflare Workers는 재배포마다 청크 해시가 바뀌는 구조라 실제 발동 가능
...
## 세션 요약 (오래된 항목은 `docs/session-log.md` 참고, (27)까지 이동 완료)
