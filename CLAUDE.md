...
## TODO
### 다음 세션 최우선
- [ ] 🔴 **실브라우저 최종 확인 필요 2건**(자동화 도구 환경 제약으로 자동검증 불가): ①**관리자 웹 푸시 OS 알림 표시**(2026-08-28 격리 docker E2E로 파이프라인 전체 재확인 — 로그인→구독저장→예약제출→백그라운드 발송 시도까지 실측 완료, `SendNotificationAsync` 실제 호출 확인. **남은 건 실브라우저의 OS 알림 표시 자체뿐**) ②**Popover(DatePicker) 닫힘 애니메이션**(2026-08-27) — `document.hidden=true`에서 `opacity`/`pointer-events` 미해제 관측(`data-state`는 정상 전환), 코드는 순정 shadcn 생성 그대로라 실사용자 환경 재현 여부만 확인하면 됨
...
## 세션 요약 (오래된 항목은 `docs/session-log.md` 참고, (62)까지 이동 완료)
...
- **2026-08-28 (64)** — **IP 레이트리밋·웹푸시 알림 전수감사(파일 전문 정독 26개+grep대조) + 격리 docker E2E 실측 + 종 아이콘 UX 개선**. 감사 결과 신규 결함 0건(입력필드 3곳 일치·i18n 408키 4파일 동일·SSRF화이트리스트·감사로그제외·인덱스 전부 정상). 격리스택(`wj-verify`, 공유스택 무영향)에서 실제 로그인→구독등록→예약제출→`WebPushClient.SendNotificationAsync` 호출까지 E2E 확인(가짜 키라 암호화단계 실패는 정상, 코드결함 아님). **어드민 요청으로 UX 개선**: `layouts/admin.vue` 종 아이콘 — 꺼짐(default/미구독)은 `text-secondary`+우측상단 `animate-ping` 점으로 강조, denied는 `text-muted-foreground`로 가라앉힘, 켜짐은 기존 `text-primary`+`fill-current` 유지. Tailwind 클래스 전부 실제 색상값(oklch) 해석되는지 브라우저 `getComputedStyle`로 실측 확인. 배너 대신 아이콘 강조를 사용자가 선택(배너/아이콘 트레이드오프 제시 후 결정).
