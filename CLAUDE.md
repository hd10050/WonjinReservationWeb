...
## TODO
### 다음 세션 최우선
- [ ] **🔴 실브라우저 시각 확인 2건 대기**(이 자동화 pane은 화면 미표시로 픽셀 렌더링 확인 불가, `list_connected_browsers`로 재확인해도 연결된 Chrome 없음 — 2026-08-27) — ①어드민 사이드바 모바일(<768px) `translateX` 슬라이드 애니메이션이 실제로 부드럽게 열리는지 ②예약 폼 날짜·시간 `<input>`의 팝업 캘린더가 로케일별로 실제 어떤 언어로 뜨는지(요일·월 이름 — `lang` 속성 무관 브라우저/OS 종속 한계로 추정, `[미확인]`)
- [ ] **`AssignConsultant` 동시 재배정 시 로그 정확성 — RowVersion 도입 필요**(보안감사 2026-08-26, Medium 부분개선만 완료) — 현재는 트랜잭션 통일까지만 적용, 동시 재배정 시 `reservation_logs`의 "이전 담당자명"이 부정확할 수 있는 근본 해결은 낙관적 동시성 토큰(스키마 변경) 필요. 최종 배정 상태 자체는 항상 정확해 긴급도 낮음
...
| 9 | SEO·보안감사·배포 | 라이브 curl 검증 — **인프라 배포는 2026-08-26 완료**(Cloudflare Workers+Render). **보안감사 1라운드**(2026-08-26, High2·Medium5 수정) + **SEO 파트**(2026-08-27, useSeo·hreflang버그·sitemap버그·OG이미지) **로컬 검증까지 완료**(세션요약 (27)(28)). Low 12건 보류 + 실배포 도메인 대상 라이브 curl 검증만 미착수 |

## 미결정 (상세: `docs/design.md` 20장)
- [ ] **M12 OG 공유 이미지 — 임시본 완료(2026-08-27), 정식 디자인은 미정** — `logo.svg` 기반 1200×630 PNG(`frontend/public/og-image.png`) 생성해 연결 완료(og:image 실측 200 확인). 마케팅팀 정식 디자인으로 교체할지는 별도 결정 필요
