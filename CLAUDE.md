...
- **2026-08-28 (54)(55) — 날짜·시간 입력 UI 4건(시간 0패딩+폭 / 날짜 년·월 select / 검색필터 역전방지 / 조회기간 1년+1일 상한) + 세션 전체 재점검·결함 수정, main 직접 작업, 상세는 `docs/session-log.md` (54)(55)**: `TimePicker`(reka-ui 기본 `'numeric'` 포맷이라 0패딩 안 됨 → `padStart` 보정 + `w-fit`) · `DatePicker`(`Calendar.vue`의 기존 `layout="month-and-year"` 활성화, `year-range` `today()` 고정으로 재선택마다 목록 안 움직이게) · 신규 공용 컴포저블 `useDateRangeFilter.ts`(종료일 `:min-value`로 역전 차단 + `rangeTooLong`으로 1년+1일 초과 시 안내문구+버튼비활성+조회차단, 5개 필터 페이지 적용) 전부 브라우저 실측 완료. **재점검 중 결함 발견·수정**: 위 방어는 필터 폼에만 걸려 `route.query` URL 직접조작이 우회 가능(미페이징 통계 API가 그대로 장기간 집계 — DB 쿼리 성능 원칙 소지) → `utils/datetime.clampDateRangeEnd()`로 SSR 프리로드에 실제 쓰이는 `query` 자체를 재차 clamp, `?from=2020-01-01&to=2026-08-28` 직접 진입 시 실조회값이 `2021-01-02`로 정정됨을 실측 확인. `npm run build` 0에러(양쪽 커밋), i18n 4파일 412키 동일 스크립트 확인. 커밋 `3e0481c`→`e6a66d9`, main push 완료. 검증용 Admin 계정(`verify-daterange@wonjin.local`)은 사용 후 매번 재정지(`is_suspended=true`) 처리.
