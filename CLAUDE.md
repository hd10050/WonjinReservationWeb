...
## 세션 요약 (오래된 항목은 `docs/session-log.md` 참고, (61)까지 이동 완료)
...
- **2026-08-28 (63)** — **IP 레이트리밋 진짜 근본원인 발견+수정, main 직접 작업(커밋 `e79e940`·`d2f500a`)**. (62)에서도 여전히 미작동해 임시 진단 엔드포인트(`/api/internal/debug-ip`, 내부시크릿 게이트, 확인 즉시 삭제)를 배포해 Render가 실제 받는 헤더값을 직접 확인 — `cfConnectingIpHeader`가 매 요청 `104.23.x.x`(Cloudflare 소유 대역)로 관측돼 내가 보낸 값이 아니었음. **진짜 원인**: Render(onrender.com)도 Cloudflare 엣지 뒤에 있어서, `CF-Connecting-IP`라는 예약된 이름의 헤더는 Render 앞단 엣지가 위조방지 목적으로 어느 발신지에서 오든 항상 실제 TCP 접속값(Workers 아웃바운드 IP, PoP마다 달라짐)으로 재작성해버림 — 지난 (61)의 "헤더 이름 불일치" 수정은 진단이 틀렸던 게 아니라 그 수정 자체가 새로운 예약 헤더 재작성 문제를 만든 것. 헤더 이름을 비예약 커스텀 이름 `X-Wj-Client-Ip`로 교체(프론트 `server/api/[...].ts`, 백엔드 `Program.cs GetClientIp()`·`AuditLogFilter.cs`, 전부 동일 이름 사용 확인). 라이브 재검증: 같은 실IP 연속 POST 6회째 정확히 429 확인(4회 재시도 끝에 재현, 이전 window 잔존값 때문). design.md 7-5·14장 갱신, memory 갱신 필요([[rate-limit-trusted-ip-only]] — Cloudflare 예약 헤더 재작성 패턴 추가).
