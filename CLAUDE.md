...
- **2026-08-25 (2) — 설계 확정 3건 반영 + 설계 자체검토**: ①배포 브랜치 `main` 확정(M1 해소) ②예약금 통화 CNY(기본)/KRW 선택 추가(D12 신설, `deposit_currency` 컬럼 — 환율 환산은 하지 않음) ③법적 검토는 사용자 확인 완료로 18장을 리스크 목록에서 "재론 금지"로 대체. 이어 design.md 전문 자체검토로 결함 11건 발견(F1~F11) — 🔴실제결함 6건(달력이 `Visited` 누락, 4카드 쿼리 설명 모순, **타임존 정의 전면 누락**, 예약코드 동시생성 중복, 랜딩 방문자마다 불필요 `/me` 401, SSR 동기 방문기록으로 랜딩 지연) · 🟡정책공백 3건(계정 삭제 정책, 실장 간 예약 접근범위, 상담기록 덮어쓰기) · 🔵성능 2건(유입경로 인덱스 컬럼순서, 방문수 조작 가능성). **수정 승인 대기 중**
