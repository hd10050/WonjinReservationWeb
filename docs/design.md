...
| D15 | **중복 신청 허용 + ~~상담 기록 없는 예약은 실장이 소프트 삭제 가능~~ → 삭제 폐지, 취소로 통일(D24)** | 같은 위챗ID로 여러 번 신청해도 막지 않는다(광고 랜딩은 실수 중복 제출이 흔하고, 막으면 진짜 재문의까지 막힌다). 삭제(소프트 삭제 포함) 관련 부분은 2026-08-27 D24로 대체 — 아래 참고 |
| D16 | **예약 통계 기간 단위는 주(일요일~토요일)** | KST 기준. PostgreSQL `date_trunc('week', …)`는 **월요일 시작**이므로 하루 밀어 계산해야 한다(11-4절) |
| D17 | **🔴 실장 배정은 수동이며, 미배정 예약에는 아무 작업도 할 수 없다** | 자동 라운드로빈 배정을 만들지 않는다(M5 해소). `consultant_id`가 NULL인 예약은 **담당 배정·취소·조회만**(2026-08-27, D24 이전엔 "소프트 삭제") 가능하고 상담 기록 추가·상태 전이(취소 제외)·방문일시/시술/예약금 저장은 전부 차단된다 — "누가 책임지는지 정해지지 않은 건에 업무 기록이 쌓이는 상태"를 원천 차단하기 위함. **담당자 변경은 예외 없이 처리 이력에 남긴다**(이전 담당자 → 새 담당자) |
| D24 | **예약 삭제 기능 폐지 — 취소+복구로 통일**(D15 대체, 2026-08-27) | 소프트 삭제(D15)는 복구 화면이 없어 사용자 입장에서 사실상 되돌릴 수 없는 액션이었다. `DELETE /{id}` 엔드포인트를 완전히 제거하고, 담당자 유무와 무관하게 정리 수단을 **취소(`Cancelled`)로 통일** — 취소는 처리 이력에 남고 **어드민만 `POST /{id}/restore`로 복구 가능**(취소 시점의 방문일·상담기록 유무로 되돌아갈 상태를 계산 — 상세 10-1절). `deleted_at`·`deleted_by_user_id` 컬럼과 전역 쿼리 필터는 과거 데이터 호환용으로 DB에 남아 있으나 이 컬럼을 채우는 쓰기 경로는 더 이상 없다 |
...
| 시각화 | `vue-chartjs` + `chart.js` | 실장 KPI·예약 통계 차트(D21). Canvas 기반 — `<ClientOnly>` 필수, SSR 불가 |
...
- ~~최초 로그인 시 클라이언트가 감지한 로케일(`wj_lang`)을 로그인 요청 body에 함께 보내, 서버는 `users.locale`이 비어 있을 때만 채운다~~ **M11 최종 결정(2026-08-26): 자동 반영 포기, `PATCH /api/auth/me/locale` 수동 변경만 유지** — `users.locale`이 `NOT NULL DEFAULT`라 "비어있음"을 구분할 수 없어 원문 그대로 구현 불가능함을 발견(Phase 1). 브라우저 감지 자체는 가능하지만 "최초 1회만 반영"을 걸 별도 컬럼이 필요한데, 직원 3역할뿐이고 대부분 한국 사무실 PC(브라우저도 한국어일 가능성)에서 근무한다는 전제(5-4절 위 문단)를 감안하면 자동화 실익이 낮아 스키마 변경 없이 종결.
...
| `deleted_at` | timestamptz | NULL — **소프트 삭제**(D15, 🔴 2026-08-27 D24로 폐지 — 이 컬럼을 채우는 쓰기 경로가 더 이상 없다. 과거 데이터 호환용으로만 컬럼·필터 유지). NULL이 아니면 모든 조회에서 제외 |
| `deleted_by_user_id` | int | NULL, FK → `users.id` ON DELETE SET NULL — 위와 동일하게 D24 이후 신규로 채워지지 않음 |
...
>
> 🔴 **소프트 삭제는 전역 쿼리 필터로 강제한다**(D15, 🔴 2026-08-27 D24로 신규 삭제 자체는 폐지 — 필터 메커니즘과 이 원칙은 과거 데이터 호환을 위해 그대로 유지). 조회마다 `WHERE deleted_at IS NULL`을 손으로 붙이면 통계·달력·KPI 중 한 군데를 반드시 빠뜨린다.
>
> ```csharp
...
고객은 중국·대만에 있고 실장은 한국에 있으므로, 기준 시간대를 적지 않으면 입력한 시각이 누구 기준인지 어긋난다. 폼 라벨에 **"연락 희망 시각 (한국시간 UTC +9)" / "希望聯絡時間（韓國時間 UTC+9）"**처럼 병기한다(2026-08-26 사용자 지시로 "한국 시간"만으로는 시차 계산이 안 되는 외국 고객을 위해 UTC 오프셋 명시로 보강, 4개 로케일 전부 동일 취지로 갱신 — `landing.form.contactTime`).
...
| GET | `/api/admin/stats/reservations` | 예약 통계 — **주(일~토) 단위** 접수·확정·방문·취소 추이, 시술별 집계, 언어별 분포, **담당 실장별 집계**. 실장 축은 **비활성 실장 제외**(D13), 시술별·언어별과 동일하게 건수만 집계하고 KPI(`/stats/consultants`)와 달리 0행 채움은 하지 않음(실적 있는 실장만) |
...
| 경로 | 화면 | 비고 |
|---|---|---|
| `/`, `/zh-tw`, `/en`, `/ko` | 홈 | 헤더 + 히어로(축소) + 카테고리 11개 바로가기 그리드 + 병원소개(콘텐츠 미확보 시 "준비 중") + 푸터. UTM 캡처·`landing-visit` 방문기록은 레이아웃(`layouts/landing.vue`)에서 처리해 딥링크 유입도 포착한다 |
| `/procedures/[category]` (4언어) | 시술 카테고리 페이지(11개) | 히어로 + 카테고리 소개 + 시술 목록(사진+고민불릿+이름, 클릭 이동 없음) + "그 외" 텍스트 배지(비이동). 🔴 **시술 상세 페이지(`/procedures/[category]/[procedure]`) 폐지**(D27, 2026-08-31) — 목록 항목이 더 이상 상세 페이지로 링크하지 않는다, 관련 `findProcedure` 헬퍼·sitemap 시술별 URL·`comingSoon` i18n 키 전부 제거 |
| `/inquiry` (4언어) | 상담 신청 폼 | 옛 랜딩(`/`)에 있던 폼을 이전(2026-08-27 재설계) — 상시 노출되는 하단 우측 FAB에서도 진입 |
| `/privacy` (4언어) | 개인정보 처리방침 | **예문 작성 완료**(2026-08-27) — 법률 검토 전 임시 문안, 최종 확정본은 여전히 범위 외(20-1절) |
| (라우트 아님) | 404 / 500 공통 에러 — Nuxt `error.vue` | 아래 |

> 🔴 2026-08-27 랜딩 재설계로 위 5개 공개 라우트 구조로 변경 — 이전엔 `/`(4언어) 한 페이지에 히어로+폼이 전부 있었다. 상세: `docs/superpowers/specs/2026-08-27-landing-redesign-design.md`.
...
**`/inquiry` 상담 신청 폼 필드**(2026-08-27 재설계 전엔 `/`에 있었음)
...
### 12-1-1. 병원 정식 정보 · 로고 확정 (M8·M10, 2026-08-26)

**M8 — 사업자등록증 등록 정보**(사용자 제공)

| 항목 | 값 |
|---|---|
| 상호(의료기관명) | 원진성형외과의원 |
| 대표자명 | 강문석 |
| 사업자등록번호 | 824-67-00414 |
| 주소 | 서울시 서초구 강남대로 419 파고다타워 12-18층 |
| 대표전화 | 성형외과 02-3477-3300 / 피부과 02-3477-0808 |

> 🔴 **대표전화는 화면에 노출하지 않는다**(2026-08-26 사용자 결정). 이 시스템은 예약 폼을 거쳐야 이윤이 발생하는 구조라, 전화번호를 화면에 크게 두면 예약 폼을 우회해 바로 전화하는 고객이 늘어 예약 시스템 이용률이 떨어질 수 있다. 대신 **JSON-LD(`MedicalClinic`, 검색엔진 전용 메타데이터)에만** 포함해, 사람 눈에는 안 보이되 최소한의 사업자 정보 요건은 채운다. 화면 푸터에는 상호·사업자번호·주소만 작게 표기(신뢰도 확보 목적 — 연락처가 전혀 없으면 광고 랜딩 특성상 오히려 의심을 사 전환율이 떨어질 수 있다). 전화번호는 두 과 중 **성형외과 번호만** 사용(이 시스템 자체가 성형외과 예약 전용).
>
> 상호·사업자등록번호는 사업자등록증 원문(한국어)을 4개 로케일 화면에 번역하지 않고 그대로 표기한다(5-6절 고유명사 원형 유지 원칙과 동일 근거 — 법적 등록 명칭은 번역하면 그 표기 자체가 달라진다).
>
> 🔴 **주소만 예외(D22, 2026-08-26)** — 화면 표기 문구를 로케일별로 분리한다: ko는 위 표의 등록원문 유지, zh-CN·zh-TW·en은 별도 문구(`landing.vue`의 `ADDRESS_BY_LOCALE` 참고, zh-TW도 영문 표기 사용). JSON-LD `PostalAddress`도 영문 주소 형식으로 통일해 갱신했다.

**M10 — 로고 이미지**(`frontend/public/logo.svg`, `favicon.png`, 사용자 제공)

- `logo.svg`: 가로형 워드마크(112.49×30 비율, 단색 `#3a5554`) — 랜딩 헤더·관리자 헤더·JSON-LD `logo`/`image` 필드에 사용.
- `favicon.png`: 32×32 — `nuxt.config.ts`의 `app.head.link`에 `rel="icon"`으로 등록. 기존 `favicon.ico`는 구형 브라우저 폴백용으로 유지.
- **OG 공유 이미지는 이번에 연결하지 않는다** — `favicon.png`(32×32)는 소셜 미리보기용으로는 해상도가 지나치게 낮고, `logo.svg`는 다수 소셜 플랫폼(페이스북 등)이 OG 이미지로 SVG를 지원하지 않는다(일반적으로 알려진 제약, 배포 후 실제 공유 테스트로 재확인 필요 — `[미확인]`). OG 전용 이미지(권장 1200×630, PNG/JPG)가 별도로 필요하며, 이 결정은 M10에서 분리해 미결정으로 남긴다.
...
- 폭 숫자 중복 문제는 `style="--sidebar-w: 220px"`(부모 요소) + `w-[var(--sidebar-w)]`(사이드바) + `md:ms-[var(--sidebar-w)]`(본문)로 해소 — 숫자를 CSS 변수 하나로만 정의해 두 소비처가 항상 동기화된다.
- 768px 이하에서는 사이드바를 `translateX(-100%)`로 숨기고 햄버거 + 오버레이로 전환한다. 🔴 **Tailwind v4의 `md:translate-x-0` 계열 유틸을 `:class` 삼항연산자로 토글하는 방식은 실제로 CSS가 생성되지 않는 것을 실측으로 확인**(빌드된 CSS에 해당 임의값 클래스 규칙 자체가 없었음 — 정적 클래스 목록에 있는 것과 삼항연산자 표현식 안의 리터럴 문자열은 Tailwind JIT 스캐너가 다르게 취급하는 것으로 추정, `[미확인]` 근본원인). **`admin.vue`의 `<style scoped>` 블록에 순수 CSS로 직접 구현**(`.admin-sidebar{transform:translateX(-100%)}` + `.admin-sidebar--open{transform:translateX(0)}` + `@media(min-width:768px)` 오버라이드)해 우회.
- 사이드바 아이콘은 `@lucide/vue` SVG 컴포넌트를 사용(D19 이전 이모지 아이콘 가정 하에 쓰인 "고정 폭 슬롯" 문구는 SVG에는 해당 없음 — lucide 아이콘은 렌더 폭이 항상 균일해 `size-4 shrink-0`만으로 충분).
...
- 메뉴 구성·역할별 노출은 루트 CLAUDE.md 역할×메뉴 권한표 그대로(6-2절과 동일) — Consultant는 대시보드·달력만, HospitalManager는 계정관리·로그·유입경로만 제외, Admin은 전부. 라벨은 `admin.nav.*` i18n 키(4개 로케일 전부 등록)로 다국어 적용.
- 상단바 언어 선택은 헤더 드롭다운(12-1절)과 달리 `<select>`(네이티브)로 구현 — `setLocale()` 즉시 반영 + `PATCH /api/auth/me/locale`로 계정에 저장(저장 실패해도 화면 전환 자체는 막지 않음).
...
7. **액션**: 저장 / 담당 실장 배정·변경 / 상태 전이(방문완료 · 취소, 사유 입력 필수) / **되돌리기**(아래, D24)

> 🔴 **삭제 기능은 없다**(D24, 2026-08-27 — D15의 소프트 삭제를 폐지하고 대체). "중복·장난 신청으로 들어온 빈 예약 정리"라는 원래 용도도 취소(`Cancelled`)로 통일했다 — 미배정 예약도 취소로 정리한다(10-1절).
>
> **되돌리기 버튼은 상태가 `Cancelled`일 때만 노출한다**(어드민 전용). 취소 시점의 방문일·상담기록은 그대로 남아있으므로, 되돌아갈 상태는 순방향 전이 규칙과 같은 기준으로 서버가 계산한다 — 방문일이 있으면 `Confirmed`, 상담 기록이 있으면 `Consulting`, 둘 다 없으면 `New`(무조건 `New`로 되돌리면 이미 진행된 상담·확정 이력이 사라져 보인다).
...
| 5 | ✅ 예약 달력(`/admin/calendar`, 좌측 월간 그리드 + 우측 선택일 목록, 라이브러리 없이 자체 구현) — **2026-08-26 완료(`session-work-2` 워크트리)** | **month<1\|>12·year 범위 밖 400 실측**(자체 DateOnly 생성 예외 처리) + **`ix_reservations_visit_date` 부분 인덱스 Bitmap Index Scan 실사용 EXPLAIN 확인**(합성 데이터 5,000건) + 브라우저 E2E(로그인→월 이동→일자 선택→상세 이동) 전건 실측 완료 |
...
| 예약 취소와 상담 기록 추가(D24, 2026-08-27 — D15 소프트 삭제 폐지로 대체된 시나리오) | 취소 요청과 상담 기록 추가를 동시 실행 | 방금 쓴 상담 내용이 잠긴(Cancelled) 예약에 조용히 저장된다 |
...
| M12 | **OG 공유 이미지**(12-1-1절 발견) — 현재 로고 파일(`favicon.png` 32×32, `logo.svg`)은 소셜 공유 미리보기용으로 부적합(해상도·SVG 미지원). 1200×630 권장 PNG/JPG 별도 제작 필요 | Phase 9(SEO 작업 시) |

> **확정되어 이동한 항목**: M1(배포 브랜치 = `main`) → 4-4절 / M4(예약금 통화 = CNY/KRW, 기본 CNY) → D12 / **M3(예약 코드 = `YYYYMMDD`+4자리, 일별 리셋) → 8-11절** / **M5(실장 배정 = 수동, 미배정 시 작업 차단) → D17·10-1절** / M7(개인정보 보유기간) → 20-1절 범위 외 / **M9(중화권 브랜드 표기 = `WonJin`, 2026-08-26 Phase 0 진행 중 확정) → D18·5-6절** / **M8·M10(병원 정식 정보·로고 이미지, 2026-08-26 확정) → 12-1-1절**.
>
...
| 개인정보 처리방침의 법률 자문을 거친 최종 확정본 · 정확한 보유기간 수치 | 범위 외 — 예문(placeholder)은 2026-08-27 작성 완료(12-1절, `privacy.vue`+`i18n` 4파일), 실제 법률 검토는 별개 |
| DB 백업·복구 정책 | 범위 외 |
